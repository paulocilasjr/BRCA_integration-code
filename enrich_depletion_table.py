...
GRANTHAM = {
    ...
    "IF":22,  "IK":94,  "IL":5,   "IM":10,  "IN":96,  "IR":97,  "IS":113,
    "IT":89,  "IV":29,
    "KE":56,  "KI":94,  "KM":95,  "KN":28,  "KQ":26,  "KR":26,  "KT":78,
    "LF":22,  "LH":99,  "LI":5,   "LM":15,  "LP":98,  "LQ":113, "LR":102,
    "LS":115, "LV":32,  "LW":61,
    ...
    "RC":180, "RG":125, "RH":29,  "RI":97,  "RK":26,  "RL":102, "RM":94,
    "RP":103, "RQ":43,  "RS":110, "RT":86,  "RW":101,
    "SA":46,  "SC":112, "SF":155, "SG":56,  "SI":113, "SL":115,
    "SN":46,  "SP":74,  "SR":110, "ST":58,  "SW":177, "SY":144,
    "TA":71,  "TI":89,  "TK":78,  "TM":81,  "TN":65,  "TP":38,  "TR":86,
    "TS":58,
    "VA":64,  "VD":124, "VE":121, "VF":50,  "VG":109, "VI":29,  "VL":32,
    "VM":21,
...
def annotate(df):
    ...
    df['grantham'] = df['Substitution'].map(GRANTHAM)
    return df
...
def write_blocked(writer, sheet_name, res):
    ...
    for ratio,label in [
        ...
        for kind in ['top','bot']:
            ...
            # header row
            hl = pd.DataFrame({ '': [f"{label} — {kind_label}"] })
            hl.to_excel(writer,
...
def write_wilcoxon(writer, sheet_name, res15, res16, metric):
    ...
    row = 0
    # title
    pd.DataFrame({ '': ['Wilcoxon rank-sum'] }).to_excel(
        writer, sheet_name=sheet_name,
        index=False, header=False, startrow=row
    )
    row += 1

    for kind_label, kind in [('Enriched','top'),('Depleted','bot')]:
        # subtitle
        pd.DataFrame({ '': [kind_label] }).to_excel(
            writer, sheet_name=sheet_name,
            index=False, header=False, startrow=row
        )
        row += 1
        ...
        for i, ci in enumerate(cats):
            for j, cj in enumerate(cats):
                if j <= i: 
                    continue
                df1 = cat_map[ci][0][ cat_map[ci][1] ][kind][metric]
                df2 = cat_map[cj][0][ cat_map[cj][1] ][kind][metric]
...
def main():
    MASTER = 'SUPP_TABLES_BRCA12_JULY_2025_V3.xlsx'
