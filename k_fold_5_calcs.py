...
OUTPUT_NAME = "classification_results_5fold_V4.xlsx"
...
def process_table(df, table_name, writer):
    ...
    detailed_cols = ["T1", "T2", "T3", "T4", "T5", "T6"] + [f"Fold_{fold}" for fold in range(1, 6)]
    classification_results[detailed_cols].to_excel(writer, sheet_name=f"{table_name}_detailed", index=False)
    final_cols = ["T1", "T2", "T3", "T4", "T5", "T6"] + [f"Final_Fold_{fold}" for fold in range(1, 6)]
